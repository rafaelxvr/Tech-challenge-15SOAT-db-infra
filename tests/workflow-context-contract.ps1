...
foreach ($job in ($workflow -split '(?m)(?=^  [a-z][a-z0-9-]+:\s*$)')) {
    ...
    if ($job -match 'id-token: write') {
        ...
        if ($job -match 'environment: production' -and $job -notmatch "vars.PRODUCTION_DEPLOYMENT_ENABLED == 'true'") { throw 'Production job must require the organization/repository activation flag.' }
