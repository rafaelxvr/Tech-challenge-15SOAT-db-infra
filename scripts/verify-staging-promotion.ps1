...
param(
    ...
    [Parameter(Mandatory)]
    [ValidatePattern('^releases/database/staging/promotions/[a-f0-9]{40}\.json$')]
    [string]$PromotionKey,
...
try {
    ...
    if ([string](Require $promotion 'codeBuildProjectName') -cne 'oficina-phase3-oficina-db-infra-staging-deploy') { Fail 'promotion evidence names an unreviewed CodeBuild project.' }
    if ([string](Require $promotion 'codeBuildBuildId') -notmatch '^oficina-phase3-oficina-db-infra-staging-deploy:[A-Za-z0-9-]+$') { Fail 'promotion evidence has an invalid staging build identity.' }
    ...
    if ($manifestKey -cne "releases/database/staging/manifests/$sourceCommit.json") { Fail 'promotion evidence points outside the reviewed staging manifest prefix.' }
