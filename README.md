...
For an authorized staging rehearsal, run the workflow manually on `develop` and enter `STAGING_ONLY`; production remains disabled until the organization- or repository-level `PRODUCTION_DEPLOYMENT_ENABLED` variable is explicitly set to `true`. Once enabled, production still requires a push to `main` plus the protected environment, cloud-window, artifact and staging-promotion checks.
