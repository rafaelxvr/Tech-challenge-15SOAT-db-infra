...
The production job is disabled by default. It runs on a protected `main` push only when the organization- or repository-level variable `PRODUCTION_DEPLOYMENT_ENABLED` is explicitly set to the exact string `true`; an absent variable evaluates false and leaves the job skipped before it requests AWS identity or reads production inputs. The flag must be organization- or repository-level because the job-level `if` is evaluated before an environment is entered. Enabling this flag requires the separate production review and promotion prerequisites below.
